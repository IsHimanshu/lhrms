...
frappe.ui.form.on("Support Timesheet", {
	setup: function (frm) {
		frappe.require("/assets/hrms/js/ars_support/timer.js");
	...
	refresh: function (frm) {
		...
		// 				ars_support.timesheet.timer(frm, row);
		...
		// 					"seconds"
		// 				);
		// 				ars_support.timesheet.timer(frm, row, timestamp);
		// 				flag = false;
		// 			}
		// 		});
		// 		// If no activities found to start a timer, create new
		// 		if (flag) {
		// 			ars_support.timesheet.timer(frm);
		// 		}
		// 	}).addClass("btn-primary");
		// } fixing manual time entry
		if (frm.doc.docstatus < 1) {
	let button = __("Start Timer");
	...
	frm.add_custom_button(__(button), function () {
		...
		$.each(frm.doc.time_logs || [], function (i, row) {
			...
			if (flag && row.activity && !row.from_time && !row.to_time) {
				ars_support.timesheet.timer(frm, row, 0);
				row.from_time = frappe.datetime.now_datetime();
				frm.refresh_fields("time_logs");
				frm.save();
				flag = false;
		...
		frm.set_query("parent_issue", function (doc) {
			return {
				filters: filters,
			};
		});
	...
	customer: function (frm) {
		frm.set_query("issue", "time_logs", function (doc) {
			return {
				filters: {
...
frappe.ui.form.on("Support Timesheet Details", {
	...
	// 	if (row.task) {
	// 		frappe.db.get_value("Task", row.task, "issue", (r) => {
	// 			frappe.model.set_value(cdt, cdn, "issue", r.issue);
	// 		});
	// 	}
	// },
	...
	time_logs_add: function (frm, cdt, cdn) {
		if (frm.doc.parent_issue) {
			frappe.model.set_value(cdt, cdn, "issue", frm.doc.parent_issue);
		}
	},
	...
	is_billable: function (frm, cdt, cdn) {
		...
		//alculate_billing_costing_amount(frm, cdt, cdn);
		calculate_time_and_amount(frm);
	},
	...
	(frm.doc.time_logs || []).forEach(log => {
		frappe.db.get_value("Employee", frm.doc.employee, "billing_rate")
			.then(r => {
				if (r.message && r.message.billing_rate) {
					frappe.model.set_value(log.doctype, log.name, "billing_rate", r.message.billing_rate);
				}
			});
	});
}
...
function set_project_in_timelog(frm) {
	if (frm.doc.parent_issue) {
		$.each(frm.doc.time_logs || [], function (i, item) {
			frappe.model.set_value(item.doctype, item.name, "issue", frm.doc.parent_issue);
		});
	}
}
